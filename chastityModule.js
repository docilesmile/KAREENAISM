export async function loadChastityModule(supabase) {
  ...
  async function getChastityStatus() {
    ...
    if (latest.is_locked && release > now) statusP.innerText = `Locked until ${release.toLocaleString()}.`;
  ...
  function scheduleDailyReset() {
    ...
    setTimeout(async () => { await dailyReset(); scheduleDailyReset(); }, next4 - now);
  }
  scheduleDailyReset();
